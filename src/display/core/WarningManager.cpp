...
void WarningManager::evaluate() {
    ...
    const auto source = controller->getResolvedBrewSource();
    const bool bluetoothSource = source == VolumetricMeasurementSource::BLUETOOTH;
    const bool measuredSource = bluetoothSource || source == VolumetricMeasurementSource::HARDWARE_SCALE;
    const int selection = settings.getScaleSource();
    const bool expectsScale = selection == SCALE_SOURCE_HARDWARE || selection == SCALE_SOURCE_BLUETOOTH ||
        (selection == SCALE_SOURCE_AUTO && (settings.getSavedBrewScale() != "" || controller->scaleAvailability().hardwareCapable));
    ...
    active[WARNING_SCALE_CONNECTED] = expectsScale && !measuredSource;
    active[WARNING_SCALE_BATTERY] = bluetoothSource && BLEScales.hasBatteryLevel() && BLEScales.getBatteryLevel() < 20;
