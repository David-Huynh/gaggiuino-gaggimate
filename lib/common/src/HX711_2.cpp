...
// Define IRAM_ATTR safely for platforms that don't use it (like STM32)
#ifndef IRAM_ATTR
#define IRAM_ATTR
#endif

// --- Universal I/O Macros ---
// This prevents us from having to write #if/#else logic on every single line of the interrupt
#if defined(ARDUINO_ARCH_STM32)
#define SET_PD_SCK(val) digitalWriteFast(PD_SCK_PN, val)
#define READ_DOUT() digitalReadFast(DOUT_PN)
#define READ_DOUT2() digitalReadFast(DOUT2_PN)
#else
#define SET_PD_SCK(val) digitalWrite(PD_SCK, val)
#define READ_DOUT() digitalRead(DOUT)
#define READ_DOUT2() digitalRead(DOUT2)
#endif

HX711_2 *_theHX711;

// --- Constructors & Timer Initialization ---
#if defined(ARDUINO_ARCH_STM32)
...
HX711_2::HX711_2() {
    _theHX711 = this;
    // ESP32 Timer setup: prescaler 80 = 1MHz base clock (1 tick = 1 microsecond)
    _hx711ReadTimer = timerBegin(0, 80, true);
    timerAttachInterrupt(_hx711ReadTimer, &_onHX711ReadTimerInterrupt, true);
    timerAlarmWrite(_hx711ReadTimer, 2, true); // Trigger every 2 microseconds
}
#else
HX711_2::HX711_2() { _theHX711 = this; }
#endif
...
// Ensure this runs in RAM for ESP32 speed
void IRAM_ATTR HX711_2::processReadTimerInterrupt(void) {
    if (!(readStatus & 0x3FUL)) {
        if (READ_DOUT() || READ_DOUT2()) {
            return;
    ...
    if (++readStatus > 0x3FFFFFFFUL) {
        SET_PD_SCK(LOW);
        readStatus -= 0x3FFFFFC0UL;
    }
}

void IRAM_ATTR HX711_2::_onHX711ReadTimerInterrupt(void) { _theHX711->processReadTimerInterrupt(); }