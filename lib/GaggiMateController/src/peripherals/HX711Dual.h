...
class HX711Dual {
  public:
    ...
    // Block until both DOUT lines go low (data ready) or timeout elapses. Both
    // converters share SCK, so clocking one early would consume mismatched samples.
    bool wait_ready_timeout(unsigned long timeout_ms, unsigned long delay_ms = 0);

    // Fast probe: both tray channels must have a fresh synchronized sample.
    bool is_ready() const;
