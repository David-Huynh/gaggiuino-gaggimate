...
ChannelFilter::ChannelFilter()
    ...
    // Q = 0.003 g/sample (raised from 0.001 — SimpleKalmanFilter applies
    //   P = P + Q then K = P / (P + R), so higher Q lifts the steady-state
    //   gain and trims the lag on slow pours. median(5) upstream still
    //   absorbs single-sample HX711 spikes, so jitter stays bounded).
    : _kalman(0.05f, 0.05f, 0.003f) {}
...
float ChannelFilter::push(float grams) {
    ...
    if (!_primed) {
        // Seed the Kalman estimator with the first valid median — drives initial
        // updateEstimate calls toward the right ballpark within a few samples.
        // SimpleKalmanFilter has no public seed API, so we feed the same value
        // a few times to converge quickly without a visible step.
        for (int i = 0; i < 4; ++i) {
